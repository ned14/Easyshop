...
# EasyShop imports
from Products.EasyShop.interfaces import IAddressContent
...
EasyShopAddress_schema = BaseSchema.copy() + schema.copy()
EasyShopAddress_schema["title"].widget.visible = {'view':'invisible', 'edit':'invisible'}
EasyShopAddress_schema["title"].required = 0

class EasyShopAddress(BaseContent):
    """
    """
    implements(IAddressContent)
    security = ClassSecurityInfo()
    _at_rename_after_creation = True
    schema = EasyShopAddress_schema
...
registerType(EasyShopAddress, PROJECTNAME)