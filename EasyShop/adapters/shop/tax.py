...
class TaxManagement:
    ...
    def getCustomerTaxes(self):
        """
        """
        return self.context.taxes.objectValues("EasyShopCustomerTax")
        
        # Todo: Optimize        
        catalog = getToolByName(self.context, "portal_catalog")
        brains = catalog.searchResults(
            path = "/".join(self.context.taxes.getPhysicalPath()),
            portal_type="EasyShopCustomerTax",
    ...
    def getDefaultTaxes(self):
        """
        """
        return self.context.taxes.objectValues("EasyShopDefaultTax")
        shop = self.context.getShop()
        
        catalog = getToolByName(self.context, "portal_catalog")
        brains = catalog.searchResults(
            path = "/".join(self.context.taxes.getPhysicalPath()),
            portal_type="EasyShopDefaultTax",
            sort_on = "getObjPositionInParent",
        )
    ...
    def getTax(self, id):
        ...
        catalog = getToolByName(self.context, "portal_catalog")
        brains = catalog.searchResults(
            portal_type = ("EasyShopDefaultTax", "EasyShopCustomerTax"),
            path = "/".join(self.context.taxes.getPhysicalPath()),
            id = id,
        )
